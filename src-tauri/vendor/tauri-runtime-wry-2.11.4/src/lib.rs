...
// kata-patch: traits de gtk (ContainerExt, BoxExt, OverlayExt, WidgetExt,
// Cast) necesarios para el overlay de webviews hijo. Sin esto, `default_vbox`
// solo se usaba como tipo en firmas.
#[cfg(any(
  target_os = "linux",
  target_os = "dragonfly",
  target_os = "freebsd",
...
/// kata-patch: asegura que la ventana tiene un `GtkFixed` (con el webview
/// principal dentro) sobre el que flotan los webviews de páginas/pestañas.
///
/// Stock tauri empaqueta los webview hijo directamente en el `GtkBox` de la
/// ventana; al no ser un `GtkFixed`, `is_in_fixed_parent` queda en `false` y
/// `set_bounds` (posición/tamaño) se ignora en Linux. Aquí todo vive en un
/// único `GtkFixed` que hace de contenido de la ventana (el principal en la
/// posición 0,0 y cada pestaña en su propio rect con `put`), de modo que los
/// bounds se aplican exactamente.
///
/// Un `GtkFixed` no tiene ventana propia: los eventos de puntero van al
/// widget con ventana más alto bajo el puntero. Fuera del rect de una
/// pestaña caen al webview principal (el cromo sigue siendo interactivo);
/// dentro, a la pestaña. A diferencia de un `GtkOverlay`, no hay región de
/// input que tapone el cromo cuando hay hijas.
///
/// Idempotente: en la segunda llamada detecta el fixed ya creado. Solo
/// migra cuando el vbox tiene exactamente un hijo (el webview principal);
/// cualquier otro layout mantiene el comportamiento original.
#[cfg(any(
  target_os = "linux",
  target_os = "dragonfly",
  target_os = "freebsd",
...
fn ensure_children_fixed(window: &Window) -> Option<gtk::Fixed> {
  let vbox = window.default_vbox().unwrap();

  // ya migramos esta ventana: devolver el fixed existente
  for child in vbox.children() {
    if let Ok(fixed) = child.dynamic_cast::<gtk::Fixed>() {
      return Some(fixed);
    }
  }

  // solo cuando el vbox tiene exactamente un hijo (el webview principal)
  let children: Vec<_> = vbox.children();
  if children.len() != 1 {
    return None;
  }

  println!("[kata-patch] usando un GtkFixed como contenido de la ventana");
  ...
  // mantiene el webview principal ajustado al tamaño del fixed (redimensionado)
  // y re-fija en CADA asignación el "size request" mínimo del fixed al tamaño
  // que la ventana le está dando: así los webviews de las pestañas no pueden
  // agrandar la ventana con su request natural cuando una página es más alta
  // que el hueco (empujaba la barra lateral fuera de pantalla). Antes se
  // fijaba solo en la primera asignación y un tamaño inicial distinto (o una
  // reasignación tras cargar contenido alto) dejaba escapar el crecimiento.
  let main_ref = main.clone();
  fixed.connect_size_allocate(move |f, alloc| {
    f.set_size_request(alloc.width(), alloc.height());
    main_ref.set_size_request(alloc.width(), alloc.height());
    main_ref.size_allocate(alloc);
  });
...
"#,
  ...
  let webview = match kind {
    ...
    WebviewKind::WindowChild => {
      // kata-patch: cada pestaña se construye dentro del `GtkFixed`
      // compartido que envuelve el webview principal (`ensure_children_fixed`).
      // Stock tauri empaqueta los webview hijo en el GtkBox de la ventana y
      // `is_in_fixed_parent` queda en `false`, así que `set_bounds` es un
      // no-op en Linux. Con el fixed, wry pone cada webview en su rect
      // (`put` + `size_request`) y set_bounds/bounds() operan de verdad.
      // Al ser un contenedor sin ventana, los eventos fuera de cada rect
      // caen al webview principal de debajo (el cromo sigue interactivo).
      let vbox = window.default_vbox().unwrap();
      if let Some(fixed) = ensure_children_fixed(window) {
