...
pub(crate) struct InnerWebView {
  ...
  // kata-patch: el GtkFixed que contiene a esta webview, para poder moverlo
  // de forma persistente (gtk_fixed_move) en set_bounds.
  gtk_fixed_parent: Option<gtk::Fixed>,
...
impl InnerWebView {
    ...
    W: IsA<gtk::Container>,
  {
    ...
    let (is_in_fixed_parent, gtk_fixed_parent) =
  ...
  fn add_to_container<W>(webview: &WebView, container: &W, attributes: &WebViewAttributes) -> (bool, Option<gtk::Fixed>)
  where
    W: IsA<gtk::Container>,
  {
    let mut is_in_fixed_parent = false;
    let mut gtk_fixed_parent = None;
    ...
    if container_type == "GtkBox" {
      ...
      // kata-patch: guardamos el fixed para poder reposicionar la webview en
      // set_bounds con gtk_fixed_move (la posición persistente de un hijo de
      // GtkFixed se guarda en el propio fixed, no en la asignación del widget).
      gtk_fixed_parent = Some(fixed.clone());
    ...
    (is_in_fixed_parent, gtk_fixed_parent)
  }
  ...
  pub fn bounds(&self) -> Result<Rect> {
    ...
    let (allocation, _) = self.webview.allocated_size();
    bounds.size = LogicalSize::new(allocation.width(), allocation.height()).into();
    // kata-patch: la posición real del webview (esto es lo que hace que
    // setSize/setPosition no pisen la posición con (0,0) al releer bounds()).
    bounds.position = LogicalPosition::new(allocation.x(), allocation.y()).into();
  ...
  pub fn set_bounds(&self, bounds: Rect) -> Result<()> {
    ...
    if self.is_in_fixed_parent {
      // kata-patch: un hijo de GtkFixed recibe su posición y tamaño en cada
      // recálculo de layout desde la posición guardada (put/move_) y desde su
      // size_request. `size_allocate` solo es transitorio: si nada invalida el
      // fixed, un setSize posterior (que relee bounds() y reenvía todo el
      // rect) podía dejar la webview en (0,0). Ahora la posición se guarda en
      // el propio fixed (gtk_fixed_move) y el tamaño en el size_request, que
      // es lo que el layout de GtkFixed respeta siempre.
      if let Some(fixed) = &self.gtk_fixed_parent {
